# Self-Hosting NAKIME with Docker

NAKIME ships as a self-contained Next.js standalone build. This guide covers
running it with Docker / Docker Compose, deploying it as a [CasaOS](https://casaos.io)
app, and configuring the optional API keys.

> **TL;DR:** NAKIME runs fully **without any API keys**. All core feeds
...
- **`env_file: .env` (`required: false`)** — if a `.env` file exists its
  values are injected into the container; if it's missing, NAKIME still starts
  with the keyless feeds.
- **`ports: ${NAKIME_PORT:-3000}:3000`** — the web UI. The container always
  listens on 3000; the published **host** port is `NAKIME_PORT` (default
  `3000`). Set `NAKIME_PORT` in `.env` to remap it, e.g. `NAKIME_PORT=3005`
...
3. NAKIME appears on the dashboard with its icon, reachable on host port
   `3000` (or whatever `NAKIME_PORT` you set in `.env`).
...
| `SCANNER_KEY` | Shared secret; **must equal the backend's `NAKIME_KEY`** | RECON toolkit |

Without `SCANNER_URL`/`SCANNER_KEY` the RECON endpoints return `503` and the
rest of NAKIME works normally. Generate a key with `openssl rand -hex 32`.
...
|----------|---------|---------|
| `NAKIME_TELEGRAM_CHANNELS` | Comma-separated list of public Telegram channel usernames (no `@`) to scrape for the **Telegram OSINT** map layer. Overrides the curated default set. | `osintdefender,insiderpaper,aljazeeraenglish,nexta_live,war_monitor` |
| `NAKIME_PORT` | Host port the compose file publishes (container itself always listens on 3000). | `3000` |
