...
import { AlertTriangle, Shield, Zap, Eye, RefreshCw, Filter } from 'lucide-react';
...
export default function SecurityAlertPanel() {
  ...
  const fetch_ = useCallback(async () => {
    try {
      ...
      const incoming: Alert[] = d.alerts ?? [];
      const fresh = new Set(incoming.map((a) => a.id).filter((id) => !prevIds.current.has(id)));
      if (fresh.size > 0) {
        setNewIds(fresh);
        setTimeout(() => setNewIds(new Set()), 3000);
      }
      incoming.forEach((a) => prevIds.current.add(a.id));
      setAlerts(incoming);
    ...
    >
      ...
      <div className="px-3 py-2.5 border-b border-[var(--border-primary)] flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-2">
          <Shield className="w-3.5 h-3.5 text-[var(--alert-red)]" />
          <span className="text-[11px] font-mono font-bold tracking-[0.15em] text-[var(--text-heading)]">SECURITY ALERTS</span>
          {stats && stats.total > 0 && (
        ...
        <div className="flex items-center gap-1.5">
          ...
          {stats && (
            <div className="flex items-center gap-1">
              <span className="text-[8px] font-mono text-[var(--text-muted)]">W</span>
      ...
      {/* Severity counters */}
      {stats && (
        <div className="grid grid-cols-4 px-3 py-1.5 gap-1 flex-shrink-0 border-b border-[var(--border-primary)]/40">
          {(['critical','high','medium','low'] as Severity[]).map((sev) => (
            <button
              ...
              style={{
                background: filter === sev ? SEV_BG[sev] : 'transparent',
                border: `1px solid ${filter === sev ? SEV_BORDER[sev] : 'transparent'}`,
              }}
            >
              <div className="text-[11px] font-mono font-bold" style={{ color: SEV_COLOR[sev] }}>{stats[sev]}</div>
              <div className="text-[8px] font-mono text-[var(--text-muted)] uppercase tracking-wider">{sev.slice(0,3)}</div>
            </button>
          ))}
        </div>
      )}
      ...
      <div className="overflow-y-auto styled-scrollbar flex-1 min-h-0">
        {loading && (
          <div className="flex items-center justify-center py-6 gap-2">
            ...
            <span className="text-[10px] font-mono text-[var(--text-muted)]">CONNECTING TO SECURITY FEEDS...</span>
          </div>
        )}

        {!loading && visible.length === 0 && (
          <div className="flex flex-col items-center justify-center py-6 gap-2">
            <Eye className="w-5 h-5 text-[var(--alert-green)]" />
            <span className="text-[10px] font-mono text-[var(--alert-green)]">NO ALERTS — SYSTEM NOMINAL</span>
          </div>
        )}
        ...
        <AnimatePresence initial={false}>
          {visible.map((alert) => (
            <motion.div
              ...
              onClick={() => setExpanded(e => e === alert.id ? null : alert.id)}
            >
              <div className="px-3 py-2 flex items-start gap-2">
                {/* Source badge */}
                <span className="flex-shrink-0 w-4 h-4 rounded text-[8px] font-mono font-bold flex items-center justify-center mt-0.5"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-0.5">
                    ...
                    <span className="ml-auto text-[8px] font-mono text-[var(--text-muted)]">{relTime(alert.timestamp)}</span>
                  </div>
                  <p className="text-[10px] font-mono text-[var(--text-primary)] leading-snug truncate">{alert.message}</p>
                  {alert.agent && (
                    <p className="text-[8px] font-mono text-[var(--text-muted)] mt-0.5">agent: {alert.agent}</p>
                  )}
                </div>
              </div>
                ...
                {expanded === alert.id && (
                  ...
                  >
                    <div className="px-3 pb-2 grid grid-cols-2 gap-x-3 gap-y-1"
                      style={{ background: SEV_BG[alert.severity], borderTop: `1px solid ${SEV_BORDER[alert.severity]}` }}>
                      {[
                        ['SRC', alert.src_ip],
                        ['DST', alert.dest_ip],
                        ['PROTO', alert.proto],
                        ...
                        ['GROUPS', alert.groups?.slice(0,2).join(', ')],
                      ].map(([label, val]) => val ? (
                        <div key={label as string} className="pt-1">
                          <div className="text-[7px] font-mono text-[var(--text-muted)] tracking-wider">{label}</div>
                          <div className="text-[9px] font-mono text-[var(--text-primary)] truncate">{val}</div>
                        </div>
                      ) : null)}
                    </div>
                  </motion.div>
